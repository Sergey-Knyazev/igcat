package ru.biocad.ig.igcont.anno
...
class Record(id : Int, n : String, s : Int,
  ...
  def setAnnotation(pos : Int, atype : String, aval : String) : (Int, Int) = {
    ...
    if (_cont(pos) == null) {
      logger.warn("Cannot make an annotation without node! Use setNode(i, node) first. Default initialization: 0.")
      setNode(pos, 0)
    }
  ...
  def annotationOf(pos : Int) : immutable.HashMap[String, String] = {
    if (_cont(pos) == null) {
      logger.warn("Cannot get an annotation without node! Use setNode(i, node) first.")
